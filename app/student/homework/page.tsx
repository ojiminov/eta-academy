...
const STATUS_STYLE: Record<string, { bg: string; color: string; dot: string }> = {
  ASSIGNED:  { bg: "#fef3c7", color: "#b45309", dot: "#f59e0b" },
  SUBMITTED: { bg: "#dbeafe", color: "#1e40af", dot: "#3b82f6" },
  GRADED:    { bg: "#dcfce7", color: "#16a34a", dot: "#10b981" },
  LATE:      { bg: "#fee2e2", color: "#dc2626", dot: "#ef4444" },
};
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "1000px" }}>
      <div style={{ marginBottom: "1.75rem" }}>
        <h1 style={{ fontSize: "1.625rem", fontWeight: "700", color: "#0f172a", margin: "0 0 0.25rem" }}>{t("homework.studentHomework")}</h1>
        <p style={{ color: "#64748b", margin: 0, fontSize: "0.875rem" }}>{t("homework.trackDesc")}</p>
      </div>

      {/* Stat cards */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1rem", marginBottom: "1.5rem" }}>
        {[
          { label: t("homework.total"), value: homework.length, color: "var(--primary)", borderColor: "var(--primary)" },
          { label: t("payments.pending"), value: pending.length, color: pending.length > 0 ? "#f59e0b" : "#10b981", borderColor: pending.length > 0 ? "#f59e0b" : "#10b981" },
          { label: t("homework.statusSUBMITTED"), value: submitted.length, color: "#3b82f6", borderColor: "#3b82f6" },
          { label: t("dashboard.avgScore"), value: graded.length > 0 ? `${avgScore.toFixed(0)}%` : "—", color: "#10b981", borderColor: "#10b981" },
        ].map(s => (
      ...
      {loading ? (
        ...
        <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: "0.875rem", padding: "4rem", textAlign: "center", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ fontWeight: "600", color: "#0f172a" }}>{t("homework.noHomework")}</div>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
          {homework.map(h => {
            ...
            const statusLabel = h.status === "ASSIGNED" ? t("homework.statusASSIGNED")
              : h.status === "SUBMITTED" ? t("homework.statusSUBMITTED")
              : h.status === "GRADED" ? t("homework.statusGRADED")
            ...
            return (
              <div key={h.id} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.625rem" }}>
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0, marginLeft: "1rem" }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: "0.25rem", padding: "0.2rem 0.625rem", borderRadius: "9999px", fontSize: "0.7rem", fontWeight: "700", background: isOverdue ? "#fee2e2" : st.bg, color: isOverdue ? "#dc2626" : st.color }}>
                      ...
                      {isOverdue ? t("homework.overdue") : statusLabel}
                    </span>
                    {pct !== null && (
                ...
                <div style={{ display: "flex", gap: "1.25rem", fontSize: "0.75rem", color: "#94a3b8", marginTop: "0.5rem", flexWrap: "wrap" }}>
                  <span>📅 {t("homework.dueDate")}: {new Date(h.homework.dueDate).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</span>
                  {h.homework.returnDate && <span>↩️ {t("homework.returnDate")}: {new Date(h.homework.returnDate).toLocaleDateString(undefined, { month: "short", day: "numeric" })}</span>}
                  <span>{t("homework.maxScore")}: {h.homework.maxScore} {t("homework.pts")}</span>
                </div>
                ...
                {h.homework.fileUrl && h.homework.fileName && (
                  <div style={{ marginTop: "0.75rem", display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0.875rem", background: "#f0f9ff", borderRadius: "0.5rem", border: "1px solid #bae6fd" }}>
                    ...
                    <a href={h.homework.fileUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: "0.72rem", color: "#0284c7", fontWeight: "700", textDecoration: "none" }}>⬇ {t("common.view")}</a>
                  </div>
                )}
                ...
                {h.submissionUrl && h.submissionName && (
                  <div style={{ marginTop: "0.75rem", display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0.875rem", background: "#f0fdf4", borderRadius: "0.5rem", border: "1px solid #86efac" }}>
                    ...
                    <a href={h.submissionUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: "0.72rem", color: "#15803d", fontWeight: "700", textDecoration: "none" }}>{t("common.view")}</a>
                  </div>
                )}
                ...
                {h.feedback && (
                  <div style={{ marginTop: "0.75rem", padding: "0.75rem", background: "#f0fdf4", borderRadius: "0.5rem", fontSize: "0.8rem", color: "#065f46", border: "1px solid #bbf7d0" }}>
                    💬 <strong>{t("homework.feedback")}:</strong> {h.feedback}
                  </div>
                )}
                  ...
                  <div style={{ marginTop: "1rem" }}>
                    {!isExpanded ? (
                      ...
                      <div style={{ border: "1.5px solid #e0e7ff", borderRadius: "0.75rem", padding: "1rem", background: "#fafafe" }}>
                        <div style={{ fontWeight: "600", color: "#4f46e5", marginBottom: "0.75rem", fontSize: "0.875rem" }}>📤 {t("homework.uploadSubmission")}</div>
                        ...
                        <div style={{ display: "flex", gap: "0.625rem", marginTop: "0.875rem" }}>
                          <button onClick={() => handleSubmit(h)} disabled={submitting === h.id} className="btn btn-primary" style={{ flex: 1, justifyContent: "center", background: "linear-gradient(135deg,#10b981,#059669)", fontSize: "0.82rem" }}>
                            {submitting === h.id ? t("homework.submitting") : uploadData[h.id] ? `✅ ${t("homework.submitWithFile")}` : t("homework.submitNoFile")}
                          </button>
                          <button onClick={() => setExpandedId(null)} className="btn btn-secondary" style={{ fontSize: "0.82rem" }}>{t("common.cancel")}</button>
                        </div>
                      </div>
                    )}
                  </div>
